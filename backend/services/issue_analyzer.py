...
class IssueAnalyzer:
    ...
    def _build_system_prompt(self, repo_full_name: str, available_labels: List[str], issue_number: int = None) -> str:
        """构建系统提示词"""
    ...
    async def analyze_issue(
        ...
        user_message = self._build_user_message(issue_info, available_labels, collaborators)
        ...
        while iteration < max_iterations:
            ...
            if not tool_calls:
                ...
                estimated_cost = (
                    (total_prompt_tokens / 1000) * price_prompt
            ...
            if (
                ...
                if strategy_config.is_model_supports_reasoning_content(settings.openai_model):
            ...
            for tool_call in tool_calls:
                try:
                    ...
                    logger.debug(
                        f"执行工具 {tool_call.function.name} (Issue 分析)"
                    )
                except Exception as e:
        ...
        result = self._parse_analysis_result(last_content) if last_content else {
            "category": "other",
            "priority": "medium",
