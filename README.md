...
- **search_web**：搜索互联网获取最新文档、API 参考和最佳实践
- **智能决策**：AI 根据需要主动调用工具
...
- **整体评分**：代码质量评分（1-10 分）
- **分类问题**：严重问题 🔴、重要建议 🟡、优化建议 💡
- **折叠展示**：详细审查内容使用 `<details>` 折叠，保持评论简洁
- **详细摘要**：变更概述和主要发现
- **可操作建议**：提供具体的改进方案
...
- **自动打标**：高置信度标签自动应用到 Issue，不存在的标签自动创建
...
![发送正在审查中和自动打标](res/发送正在审查中和自动打标.png)

#### 审查完成 - AI 结构化审查报告

![PR审查完成示例](res/PR审查完成示例.png)
...
在 PR 中评论以下命令可与 Sakura AI 交互：

- **`/full-review`**：清理旧评论并触发全量重新审查（PR 作者或仓库协作者）
- **`/revoke`**：一键撤回所有 AI 评论和 Review，不触发重新审查（仅仓库管理员）
...
![Telegram通知示例-1](res/Telegram通知-1.png)

![Telegram通知示例-2](res/Telegram通知-2.png)
...
- [x] Issue 自动打标（高置信度标签自动应用，自动创建不存在的标签）
- [x] PR-Issue 智能关联（自动解析引用、上下文注入）
- [x] Web 搜索工具（AI 可搜索互联网获取最新文档和最佳实践）
- [x] 折叠审查报告（详细内容使用 `<details>` 折叠，保持评论简洁）
- [x] 一键撤回命令（`/revoke` 撤回所有 AI 评论和 Review）
- [x] WebUI 配置 Web 搜索参数
